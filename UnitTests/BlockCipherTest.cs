...
namespace UnitTests
{
    ...
    public class BlockCipherTest
    {
        ...
        public class CTRModeTests
        {
            ...
            {
                string input = "Yo, VIP Let's kick it Ice, Ice, baby Ice, Ice, baby ";
                CTRMode cipher = new CTRMode();
                cipher.plainText = input.toByteArray();
                cipher.key = "YELLOW SUBMARINE".toByteArray();
                cipher.encrypt();
                string output = cipher.cipherText.toBase64String();
                string answer = "L77na/nrFsKvynd6HzOoG7GHTLXsTVu9qvY/2syLXzhPweyyMTJULu/6/kXX0KSvoOLSFQ==";
                Assert.AreEqual(answer, output);
            }
            ...
            public void CTRDecryptionTest()
            {
                ...
                Assert.IsTrue(output.Contains("Yo, "));
            }
            [TestMethod]
            public void NonceTest()
            {
                CTRMode cipher = new CTRMode();
                //check for case of valid nonce
                byte[] nonce = new byte[cipher.blockSize / 2];
                for (int i = 0; i < nonce.Length; i++)
                    nonce[i] = 0x00;
                cipher.nonce = nonce;
                Assert.AreEqual(cipher.blockSize / 2, cipher.nonce.Length);
                int nonceSum = 0;
                foreach (byte b in cipher.nonce)
                    nonceSum += b;
                Assert.AreEqual(0, nonceSum);

                //check for short IV size
                string nonceString = "YELLOW";
                try
                {
                    cipher.IV = nonceString.toByteArray();
                    Assert.Fail("IV size is incorrect at set");
                }
                catch (InvalidLengthIV e)
                {
